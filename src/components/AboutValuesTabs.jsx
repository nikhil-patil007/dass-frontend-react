...
const AboutValuesTabs = ({ tabs = DEFAULT_TABS, delay = AUTOPLAY_DELAY }) => {
    ...
    const previous = useMemo(() => tabs[prevIndex % count], [tabs, prevIndex, count]);
                ...
                <div className="text_content m-auto">
                    <div className="inner-flex inner-flex-small">
                        {tabs.map((t, i) => (
                            <h2
                                key={t.key}
                                className={`tab-item ${i === index ? "is-active" : "is-inactive"}`}
                                onClick={() => goTo(i)}
                            >
                                {t.label}
                            </h2>
                            ...
                            {tabs.map((t, i) => {
                                ...
                                return (
                                    <img
                                        ...
                                        src={t.image}
                                        alt={t.title}
                                        className={`stack-img ${isActive ? "is-active" : isPrev ? "is-prev" : "is-idle"}`}
                                        loading="lazy"
                                    />
                                );
                            })}
